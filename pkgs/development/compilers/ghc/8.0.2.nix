...
, # What flavour to build. An empty string indicates no
  ...
  ghcFlavour ? lib.optionalString (stdenv.targetPlatform != stdenv.hostPlatform) "perf-cross"
}:
...
let
  ...
  libDeps = platform: [ ncurses ]
    ...
    ++ lib.optional (platform.libc != "glibc") libiconv;
...
stdenv.mkDerivation rec {
  ...
  ] ++ lib.optional stdenv.isLinux ./ghc-no-madv-free.patch
    ++ lib.optional stdenv.isDarwin ./ghc-8.0.2-no-cpp-warnings.patch
    ++ lib.optional stdenv.isDarwin ./backport-dylib-command-size-limit.patch;
  ...
  ] ++ lib.optional (targetPlatform == hostPlatform && ! enableIntegerSimple) [
    "--with-gmp-includes=${targetPackages.gmp.dev}/include" "--with-gmp-libraries=${targetPackages.gmp.out}/lib"
  ] ++ lib.optional (targetPlatform == hostPlatform && hostPlatform.libc != "glibc") [
    "--with-iconv-includes=${libiconv}/include" "--with-iconv-libraries=${libiconv}/lib"
  ] ++ lib.optionals (targetPlatform != hostPlatform) [
    "--enable-bootstrap-with-devel-snapshot"
  ] ++ lib.optionals (targetPlatform.isDarwin && targetPlatform.isAarch64) [
